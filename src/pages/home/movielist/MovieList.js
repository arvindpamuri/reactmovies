...
const MovieList = ({movieDisplayData, setCurrentMovieNo}) => {
    ...
    if(Object.keys(movieDisplayData).length !== 0 ) {
        ...
        const lst = movieDisplayData.map((movie) => {
            ...
            return(
                <Col className="col-xs-12 col-sm-6 col-md-4 d-flex justify-content-center" key={movie.No}>
                
                    <Card bg="secondary" className="movie-card" style={{ width: '16rem', minHeight: '150px', margin: '20px 0'}}  onClick={() => handleMovieClick(movie.No, movie.Title)}>
